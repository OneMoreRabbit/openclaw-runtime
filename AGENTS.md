...
This repo is the **agent-image** component of AgentEco, governed by Architecture-Above-Code.
...
**While working:** you may write only to `components/agent-image/**`, an additive
...
need something owned elsewhere, ask for it in `components/agent-image/docs/needs/`.
...
`atlas/agent-image/<topic>`, open the PR).
